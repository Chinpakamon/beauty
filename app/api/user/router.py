...
async def user_registration(
    ...
    return await service.UserService.registration_user(
        data=data, 
        session=session
    )
...
async def login(
    data: schemas.LoginUserRequestSchemas, 
    session: AsyncSession = Depends(get_session)
):
...
async def user_list(
    ...
    return await service.UserService.get_user_list(
        data=data, 
        session=session
    )


@router.patch("/change-password", response_model=schemas.UpdateUserPasswordResponseSchemas)
